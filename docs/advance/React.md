...
[[toc]]
## React简介
React是一个专注于视图的JavaScript库，通过引入虚拟DOM机制，减少了真实DOM的操作，提高了性能。使用JSX拓展了JS语法，表达更加灵活。
自带组件化方案，使代码更加容易复用。使用了合成事件抹平了不同浏览器上的兼容差异。推崇单向数据流，数据管理清晰。
...
关于`Reconciliation`的具体过程详见[React的Fiber原理](#react的Fiber原理)
...
引入`Fiber`之后，可以将`reconcile`拆分成更小的任务，`reconcile`过程可以暂停，通过`requestIdleCallback`执行任务，分阶段完成。并将任务按照优先等级划分，优先执行高优先级的任务(比如浏览器渲染任务)。

![FiberTree](./img/fiber_tree.png)

1. 第一次`render`时，调和过程会根据`ReactElement`创建第一棵树`Current Fiber Tree`，树中的每个节点都是`Fiber`节点，每个`Fiber`节点是一个任务单元，单个任务完成后可以暂停或继续下一个任务。可以看出，整棵树利用了链表结构，父节点`child`属性链接到第一个子节点，子节点的`return`属性链接到其父节点，同时子节点的`sibling`属性链接了子节点的兄弟节点。一个Fiber节点的部分属性如下：
    ```typescript
    Fiber = {
      tag: WorkTag,
      key: null | string,
    ...
    ```
2. 后续`update`被触发时，React会从`Host Root`开始递归创建新的`WorkInProgress Tree`，新旧DOM树的节点通过`alternate`属性关联。在创建`WorkInProgress Tree`过程中，如果节点没有更新，会复用旧`current tree`上的节点，如果有更新，则调用组件的`render`生成ReactElement，然后与`current tree`上的节点对比(即`diff`过程)。在递归的返回阶段(`completeWork`)，会从子级到父级依次合并`effect list`(所以`effect list`顺序是自下而上)，返回到`Host Root`递归结束。

3. commit阶段：遍历`effect list`，执行`side effects`，将更新应用到真实DOM上

### React的Diffing算法
- 当对比两棵树时，React 首先比较两棵树的根节点。不同类型的根节点元素会有不同的形态。
- 当根节点为不同类型的元素时，React会将整个树重建
- 对比两个相同类型的`React Element`时，React 会保留 DOM 节点，仅比对及更新有改变的属性
- 对比同类型的组件元素(`React Component`)时，组件实例会保持不变，仅更新props并触发相应的更新生命周期，然后调用`render`函数并对对`render`后的结果进行diff
- 在新旧元素的`key`不等时，React也会更新该节点，这在同级多个节点的Diff时尤其有用。见[React中的key的作用是什么](##react中的key的作用是什么)
