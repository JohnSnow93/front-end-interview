...
- 回流（重排）:当render树中的一部分或者全部因为尺寸或位置改变而需要DOM树重新计算的过程。

重绘不一定导致回流（比如颜色的改变），回流必然导致重绘（比如网页元素改变位置）。
## 怎么让Chrome支持小于12px的文字
1. 使用CSS的`transform`缩小文字：`transform:scale(0.8)` 
2. 使用私有属性`-webkit-text-size-adjust:none;` 缺点是chrome更新到27版本之后就不可以用了
3. 将小于12px文字内容切出做图片
...
## 前端常用的CSS优化手段
### 1. 加载性能
- CSS合并与压缩：将写好的CSS进行打包压缩，可以减少很多的体积
- 减少使用@import,而建议使用link，原因详见[link和@import有什么区别](#link和@import有什么区别)
- 使用`CSS Sprites`和 `base64` 格式图片来减少网络请求次数
- CSS单一样式：当需要下边距和左边距的时候，很多时候选择:`margin: 上下外边距 左右外边距`但只写`margin-bottom`和`margin-left`执行的效率更高
...
- 避免页面重绘(`repaint`)和回流(`reflow`)
- 选择器优化嵌套，尽量避免层级过深
- 外部样式引入放到`<header>`中，避免发生[FOUC](#什么是FOUC?如何避免)
### 3. 可维护性、健壮性
- 将具有相同属性的样式抽离出来，提高CSS的可维护性。
- 样式与内容分离：将样式代码定义到外部CSS中。
